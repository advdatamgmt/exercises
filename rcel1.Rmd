...
+ Make a really long `vector` by typing this: `1:100`.  Now that you know how to extract elements from a `vector`, explain why R has been printing `[1]` at the beginning of the output all this time.
...
rm(list = ls())                               # this will make sure your working environment is clean
                                              # so no naming conflicts arise
```

1. Is `2dogs` a valid name in R?  Why or why not?

2. What is the sum of the 11th and 82nd element of `x`?
...
```{r, opts.label='hideall'}
summary(factor(z))[3]   # did you know the oupput is a vector? is.vector()?
```