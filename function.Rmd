...
The food called **arguments** goes in the mouth separated commas (I like to think of them as teeth - OK maybe I'm taking the analogy too far...).  

Where is the body?  R functions are shy and do not like to show off their body.  They hide it, but if you call their name
without offering food, they will show it to you as they turn to run to run away.
...
+ If you cut-and-paste and realize you need to change something you have to do it everywhere, with 
...
rule of not using the same code more than three times is also know as the "rule of three" in
...
However, if you use a **uninitialized** variable within you function R can use the one from the
environment:
...
you return "two" things at once?  (Hint: use something you already know.)
...
1. Look at the `gl` function.  Give a working example where you use the arguments out of order.
...
3. Write a function that takes a character name and a data.frame and returns that column from a data.frame. Call the
function `select_column`.
```{r, opts.label='hideall'}
select_column <- function(name, df) { df[, name] }
```

4. The `length` function gives you the length of vector and `sum` adds a numeric vector together.  Write a function called `mean` that takes a vector as input.
```{r, opts.label='hideall'}
mean <- function(x) { sum(x) / length(x) }
```

5. Using your `mean` function write a function that calculates the variance called 
`var`.  Remember variance is the sum ($\sum$)
...
```{r, opts.label='hideall'}
var <- function(x) { sum((x - mean(x))^2) / (n - 1) }
```