...
l # notice the hist that R gives you about how to access a given elmeent
l[[2]]
```
...
represening data where there are several values (in columns) per observation (in rows).  You create a `data.frame` with the `data.frame()` function:
...
syntax from this exercise with that of the last one.)
...
+ Demostrate two ways to reassign the second element of `l2` to a value of `900`. Notice the difference
between `O` ("oh") and `0` ("zero").
